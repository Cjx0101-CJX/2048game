...
function init(){
	for(var i=0; i<4; i++){
		...
		for(var j=0; j<4; j++){
			...
			//通过getPostTop()方法设置每个小格子距顶端的距离
			gridCell.css("top",getPostTop(i,j));
			//通过getPostleft()方法设置每个小格子距顶端的距离
	...
	for(var i=0; i<4; i++){
		for(var j=0; j<4; j++){
			...
			if(board[i][j] == 0){
				...
				numberCell.css('top',getPostTop(i,j)+50);
				numberCell.css('left',getPostLeft(i,j)+50);
			}
			//如果棋盘格的值不为0，设置数字格为高宽75并设置背景色以及数字值
			else{
